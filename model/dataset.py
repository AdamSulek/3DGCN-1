...
class Dataset(object):
    ...
    def load_dataset(self):
        # Dataset parameters
        if (
            self.dataset == "bace_reg"
            or self.dataset == "delaney"
        ...
        for mol in mols:
            if mol is not None:
                ...
                if type(self.target_name) is list:
                    y.append(
                        [
                            float(mol.GetProp(t)) if t in mol.GetPropNames() else -1
                            for t in self.target_name
                        ]
                    )
        ...
        if self.task != "regression":
            self.mols, self.coords, self.target = (
                np.array(x),
                np.array(c),
    ...
    def save_dataset(self, path, pred=None, target="test", filename=None):
        mols = []
        for idx, (x, c, y) in enumerate(
            zip(self.x[target], self.c[target], self.y[target])
        ):
    ...
    def get_atom_features(self, mol):
        ...
        for atom_idx in range(mol.GetNumAtoms()):
            ...
            o += (
                ...
                if self.use_implicit_valence
                else []
            )
            o += (
                one_hot(atom.GetFormalCharge(), [-3, -2, -1, 0, 1, 2, 3])
                if self.use_degree
                else []
            )
