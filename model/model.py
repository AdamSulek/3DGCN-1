...
def model_3DGCN(hyper):
    ...
    for _ in range(num_layers):
        ...
        sc = GraphConvS(units_conv, pooling="sum", activation="relu")(
            [sc_s, sc_v, adjms]
        )
        vc = GraphConvV(units_conv, pooling="sum", activation="tanh")(
            [vc_s, vc_v, adjms]
        )
    ...
    vc_out = TimeDistributed(
        Dense(units_dense, activation="relu", kernel_regularizer=l2(0.005))
    )(vc)
    vc_out = TimeDistributed(
        Dense(units_dense, activation="relu", kernel_regularizer=l2(0.005))
    )(vc_out)
    ...
    if task == "regression":
        ...
        model.compile(
            ...
            loss=loss,
            metrics=[std_mae(std=std), std_rmse(std=std)],
        )
    elif task == "binary":
